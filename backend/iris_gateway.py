...
class IRISGateway:
    ...
    def _speak_response(
        self, input_source: Union[str, queue.Queue], session_id: str = None
    ) -> None:
        """
    ...
    async def _handle_monitor_card(
        ...
        try:
            ...
            if section_id == "diagnostics":
            ...
            elif section_id == "api_keys":
                # ── API Keys: load or save ────────────────────────────────
                import json as _json
                from ..iris_config import load_config as _ak_load
                from pathlib import Path as _Path

                _ak_cfg = _ak_load()
                _ak_current = {}

                # Try to load saved keys from iris_config. They live under
                # a "api_keys" key (or in inference.api_keys).
                _api_keys_section = getattr(self, "_api_keys_cache", {})
                if not _api_keys_section:
                    _saved = getattr(_ak_cfg, "api_keys", None) or getattr(
                        getattr(_ak_cfg, "inference", None), "api_keys", {}
                    )
                    if isinstance(_saved, dict):
                        _api_keys_section = _saved
                ...
                if values and isinstance(values, dict):
                    # ── Save API keys ──────────────────────────────────────
                    _saved_keys = {}
                    for _k in ("picovoice_access_key", "hf_token"):
                        if values.get(_k, "").strip():
                            _saved_keys[_k] = values[_k].strip()
                    if _saved_keys:
                        try:
                            # Persist to iris_config.json
                            _ak_cfg_path = (
                            ...
                            _existing = {}
                            if _ak_cfg_path.exists():
                                with open(_ak_cfg_path, "r") as _f:
                                    _existing = _json.load(_f)
                            _existing.setdefault("api_keys", {}).update(_saved_keys)
                            with open(_ak_cfg_path, "w") as _f:
                                _json.dump(_existing, _f, indent=2)
                            self._api_keys_cache = _saved_keys

                            # Also write to .env.local for other processes
                            try:
                                _env_local = _Path(__file__).parent.parent / ".env.local"
                                _env_lines = []
                                if _env_local.exists():
                                    _env_lines = _env_local.read_text().splitlines()
                                # Update or append
                                _updates = {
                                    "PICOVOICE_ACCESS_KEY": _saved_keys.get("picovoice_access_key", ""),
                                }
                                if "hf_token" in _saved_keys:
                                    _updates["HF_TOKEN"] = _saved_keys["hf_token"]
                                for _key, _val in _updates.items():
                                    if not _val:
                                        continue
                                    _found = False
                                    for _i, _line in enumerate(_env_lines):
                                        if _line.strip().startswith(f"{_key}="):
                                            _env_lines[_i] = f"{_key}={_val}"
                                            _found = True
                                            break
                                    if not _found:
                                        _env_lines.append(f"{_key}={_val}")
                                _env_local.write_text("\n".join(_env_lines) + "\n")
                            except Exception as _env_err:
                                self._logger.warning(
                                    f"[{session_id}] Could not write .env.local: {_env_err}"
                                )
                            ...
                            await self._ws_manager.send_to_client(
                                client_id,
                                {
                                    "type": "update_field",
                                    "section_id": "api_keys",
                                    "field_id": "api_keys_saved",
                                    "value": "saved",
                                },
                            )
                            self._logger.info(f"[{session_id}] API keys saved")
                        except Exception as _save_err:
                            self._logger.warning(
                                f"[{session_id}] API keys save error: {_save_err}"
                            )
                            await self._ws_manager.send_to_client(
                                client_id,
                                {
                                    "type": "update_field",
                                    "section_id": "api_keys",
                                    "field_id": "api_keys_error",
                                    "value": str(_save_err),
                                },
                            )
                    else:
                        self._logger.info(f"[{session_id}] API keys save: no keys provided")
                else:
                    # ── Load current keys (card auto-request) ──────────────
                    _display = dict(_ak_current)
                    # Mask values for display (keep first 4 chars)
                    for _k, _v in _display.items():
                        if len(_v) > 6:
                            _display[_k] = _v[:4] + "*" * (len(_v) - 4)
                    await self._ws_manager.send_to_client(
                        client_id,
                        {
                            "type": "update_field",
                            "section_id": "api_keys",
                            "field_id": "api_keys_data",
                            "value": _json.dumps(_ak_current),
                        },
                    )
