...
class MemoryConfig:
    ...
    def validate(self) -> bool:
        ...
        if not 0.5 <= self.compression.threshold <= 0.95:
            errors.append(
                f"compression.threshold must be 0.5-0.95, got {self.compression.threshold}"
            )

        # Validate distillation settings
        if self.distillation.interval_hours < 1:
            errors.append(
                f"distillation.interval_hours must be >= 1, got {self.distillation.interval_hours}"
            )

        # Validate retention settings
        if self.retention.episode_retention_days < 7:
            errors.append(
                f"retention.episode_retention_days must be >= 7, got {self.retention.episode_retention_days}"
            )
        ...
        if (
            self.embedding.similarity_threshold < 0.3
            or self.embedding.similarity_threshold > 0.95
        ):
            errors.append(
                f"embedding.similarity_threshold must be 0.3-0.95, got {self.embedding.similarity_threshold}"
            )
...
def update_context_size(token_budget: int) -> None:
    """Update the global config's max_context_size from the agent kernel.

    Called whenever the user selects a new model so memory budgets match the
    model's actual context window instead of a hardcoded default.
    """
    config = get_config()
    old = config.max_context_size
    config.max_context_size = token_budget
    logger.info(f"[MemoryConfig] max_context_size updated: {old} → {token_budget}")
