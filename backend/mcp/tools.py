...
class ToolRegistry:
    ...
    def __init__(self, max_history: int = 100, enable_security: bool = True):
        ...
        if self._security_enabled:
            try:
                ...
                logger.info("[ToolRegistry] Security validation enabled")
            except Exception as e:
                logger.error(f"[ToolRegistry] Failed to initialize security: {e}")
                self._security_enabled = False
        else:
            if not SECURITY_AVAILABLE:
                logger.info("[ToolRegistry] Security components not available")
            elif not enable_security:
                logger.info("[ToolRegistry] Security validation disabled by configuration")
    ...
    def register_local_tool(self, name: str, func: Callable, description: str = "") -> None:
        ...
        logger.info(f"[ToolRegistry] Registered local tool: {name}")
    ...
    async def execute_local_tool(self, name: str, arguments: Dict[str, Any], 
        ...
        if self._security_enabled and self._security_manager:
            try:
                ...
                logger.error(f"[ToolRegistry] Security validation error: {e}")
        ...
        try:
            ...
            logger.error(f"[ToolRegistry] Local tool error: {e}")
