...
def test_crawler_query_returns_content():
    ...
    # NOTE (2026-08-10, seam update called out loud): this test's old seam
    # patched CrawlerEngine + crawl_planner + data_extractor — a no-op since
    # _execute_crawler_query routes through CrawlOrchestrator.research()
    # (REQ-15 unified funnel), so the REAL subprocess crawler fetched
    # example.com over the network and this fixture never entered the funnel.
    # The seam now patches get_crawl_orchestrator() — the same seam the
    # behavioral twin (backend/tests/behavioral/test_web_mode_gate.py)
    # adopted on 2026-08-09. The fake pages carry `.html` so the REQ-1 AC1
    # `page_is_usable` predicate in the tool-boundary content collector can
    # read it without AttributeError, and markdown long enough to pass
    # MIN_CONTENT_CHARS (20). The assertions are unchanged: the requirement
    # (full rich markdown in `content`, not just the summary) is identical.
