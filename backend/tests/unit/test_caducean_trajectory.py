...
class TestCaduceanTrajectoryRecorder:
    ...
    def test_trajectory_count(self, recorder):
        ...
        recorder.record("s", 0, 0, 0, 0.0, 0.0, 1, "ok", 1.0)
        assert recorder.trajectory_count() == 1
        recorder.record("s", 1, 0, 0, 0.0, 0.0, 1, "ok", 1.0)
        assert recorder.trajectory_count() == 2

    def test_get_trajectories_in_insertion_order(self, recorder):
        recorder.record("s", 0, 0, 0, 0.0, 0.0, 1, "a", 1.0)
        recorder.record("s", 1, 0, 0, 0.0, 0.0, 1, "b", 2.0)
        recorder.record("s", 2, 0, 0, 0.0, 0.0, 1, "c", 3.0)
        rows = recorder.get_trajectories()
    ...
    def test_eml_cache_updated(self, recorder):
        ...
        recorder.record("s", 0, 0, 0, 0.0, 0.0, 1, "ok", 2.34)
    ...
    def test_get_trajectories_filter_by_session(self, recorder):
        recorder.record("sess_a", 0, 0, 0, 0.0, 0.0, 1, "x", 1.0)
        recorder.record("sess_b", 0, 0, 0, 0.0, 0.0, 1, "y", 1.0)
    ...
    def test_graceful_no_crash_on_double_record(self, recorder):
        recorder.record("s", 0, 0, 0, 0.0, 0.0, 1, "ok", 1.0)
        recorder.record("s", 0, 0, 0, 0.0, 0.0, 1, "ok", 1.0)  # should not raise
        assert recorder.trajectory_count() == 2

    def test_concurrent_writes(self, recorder):
        def worker(i):
            recorder.record(f"s", i, i * 0.1, i * 0.1, 0.0, 0.0, 1, "ok", i * 0.1)
