...
def test_behavioral_predictor_accepts_myc_arg():
    """T9 (REQ-12): BehavioralPredictor(myc) — the storing form every
    production call site (explorer.py:107, evidence.py:69, live_context.py:127,
    recall_decoder.py:464) uses — must instantiate and store the interface."""
    from unittest.mock import MagicMock
    ...
    assert bp is not None
    assert bp._myc is myc, "the constructor must STORE the mycelium interface"
