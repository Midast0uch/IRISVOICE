...
# pollution).  We ALSO never MUTATE an already-imported engine module: if
# backend.audio.engine is already in sys.modules (imported by an earlier test
# file), we must not replace AudioEngine with object. We create fresh stub
# modules only when the real ones are absent, and restore the originals right
# after loading pipeline/voice_command (mirrors test_vad_reliability.py).
import os as _os
...
_orig_modules = {
    ...
    for name in ("backend.audio", "backend.audio.engine", "backend.audio.cadence_detector",
                 "backend.audio.pipeline", "backend.audio.voice_command")
}
...
# Only set stub attributes if we actually created a stub (i.e. the real
# module was NOT already loaded); otherwise we would corrupt the real one.
if "backend.audio.engine" in _orig_modules and _orig_modules["backend.audio.engine"] is None:
...
if "backend.audio.cadence_detector" in _orig_modules and _orig_modules["backend.audio.cadence_detector"] is None:
...
# Restore the real module tree in sys.modules now that pipeline/voice_command
# are loaded (they captured their own references). Without this, the stubs
# leak into every later test file (same bug class as test_vad_reliability.py).
