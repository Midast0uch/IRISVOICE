...
def test_crawler_query_returns_content():
    ...
    class _FakeCrawlResult:
        ...
        duration_ms = 100
        pages = [
            _FakePage("https://example.com/a", "# Heading A\n\nBody A"),
            _FakePage("https://example.com/b", "# Heading B\n\nBody B"),
        ]
    ...
    with patch("backend.crawler.crawler_engine.CrawlerEngine", _FakeEngine), \
         patch("backend.crawler.crawl_planner.get_crawl_planner") as mock_planner, \
         patch("backend.crawler.data_extractor.get_data_extractor") as mock_extractor:
        mock_planner.return_value.plan = lambda *a, **k: _FakePlan()
        # plan() is awaited in the source
        async def _plan(*a, **k):
            return _FakePlan()
        mock_planner.return_value.plan = _plan
        mock_extractor.return_value.extract = _FakeExtractor().extract
