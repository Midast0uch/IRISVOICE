...
# ---------------------------------------------------------------------------
# REQ-12 (T10): caducean_trajectories.domain column (pre-domain stores)
# ---------------------------------------------------------------------------


def test_domain_column_exists_on_fresh_create():
    """T10: a fresh store gets the domain column (from _SQL_CREATE)."""
    tmp = tempfile.mktemp(suffix=".db")
    conn = sqlite3.connect(tmp)
    try:
        CaduceanTrajectoryRecorder(db_conn=conn)
        ...
        assert "domain" in cols, f"domain column missing on fresh create: {cols}"
        print(f"  PASS  domain column present (cols: {cols})")
    finally:
        conn.close()
        try:
            os.unlink(tmp)
...
def test_domain_alter_upgrades_legacy_store():
    """T10: a PRE-domain store (the app-store legacy shape, baseline-report §8)
    is upgraded by the idempotent ALTER — rows keep their values, default 'general'."""
    tmp = tempfile.mktemp(suffix=".db")
    conn = sqlite3.connect(tmp)
    try:
        # Build the LEGACY schema exactly as the app store predates the column.
        conn.execute(
        ...
        conn.execute(
            "INSERT INTO caducean_trajectories (ts, session_id, x, y) "
            "VALUES (1.0, 'legacy-sess', 0.5, 0.5)"
        )
        conn.commit()

        # The recorder's _ensure_table applies the domain ALTER.
        CaduceanTrajectoryRecorder(db_conn=conn)
        ...
        assert "domain" in cols, f"domain column not added to legacy store: {cols}"
        # Existing row preserved; domain defaults to 'general' (REQ-12 default).
        row = conn.execute(
            "SELECT domain FROM caducean_trajectories WHERE session_id='legacy-sess'"
        ).fetchone()
        assert row is not None, "legacy row must survive the ALTER"
        assert row[0] == "general", f"domain default should be 'general', got {row[0]}"

        # Idempotent: second init must not raise.
        CaduceanTrajectoryRecorder(db_conn=conn)
        print("  PASS  pre-domain store upgraded + idempotent (domain='general')")
    finally:
        conn.close()
        try:
            os.unlink(tmp)
