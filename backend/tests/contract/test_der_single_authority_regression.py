...
def test_critical_failure_routes_through_split_not_graft():
    ...
    with patch("backend.agent.event_bus.get_event_bus", return_value=MagicMock()):
        agent_kernel.AgentKernel._der_handle_step_failure.__get__(
            ...
            # Semantic-failure input per D4: REAL CONTENT is the only
            # failure that may split (error/empty results classify and
            # are recorded without fan-out). The previous no-argument
            # call (None -> empty error) now correctly stays on the
            # no-split path; this drives the split with the input that
