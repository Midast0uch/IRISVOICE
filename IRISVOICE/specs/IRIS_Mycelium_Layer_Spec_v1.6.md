...
# IRIS Mycelium Layer — Implementation Specification
...
*   **Readable Profile Layer** – the missing human face on the coordinate graph. `ProfileRenderer` derives a prose profile from the coordinate graph and landmark graph together. It is never manually written. It is always generated from the current state of the map. When coordinates shift — user relocates, domain proficiency changes, new tools become habitual — the profile regenerates automatically during distillation. The profile is the translation of the map into human language. The map is the truth. The profile is the projection.
*   **Profile stratification** – not all coordinate spaces render into the readable profile. `toolpath` is operational data — the agent uses it, the user doesn’t read it. `capability` renders as a capability summary, not raw hardware specs. `conduct`, `domain`, `style`, `chrono`, `context` render fully. The profile shows what is meaningful to a human. The rest stays in the machine layer where it belongs.
...
```mermaid
graph TD
    subgraph MemoryInterface
        A[get_task_context()] --> B[assembles all zones]
    end
...
<table>
  ...
  <tbody>
    <!-- No files listed in the provided image -->
  </tbody>
</table>
...
confirmation_threshold: 0.0 = wants confirmation on any consequential act
1.0 = never needs confirmation regardless of stak

correction_rate: 0.0 = never corrects mid-task
...
Note: Toolpath edges decay faster than user-profile edges (decay_rate=0.02 vs 0 because tool availability and task patterns change more often than identi
See EdgeScorer – toolpath edges use TOOLPATH_DECAY_RATE constant.
...
CREATE TABLE IF NOT EXISTS mycelium_landmark_merges (
    ...
    pre_merge_score_a REAL NOT NULL, -- absorbing score before merge
    post_merge_score REAL NOT NULL, -- combined score after merge
    created_at REAL NOT NULL
);
...
CREATE INDEX IF NOT EXISTS idx_epindex_episode ON mycelium_episode_index(episode_id)
CREATE INDEX IF NOT EXISTS idx_epindex_session ON mycelium_episode_index(session_id)
CREATE INDEX IF NOT EXISTS idx_epindex_landmark ON mycelium_episode_index(landmark_id)
CREATE INDEX IF NOT EXISTS idx_epindex_hash ON mycelium_episode_index(coordinate_hash)
...
def _extract_domains(self, text: str) -> List[Tuple]:
    ...
    domain_signals = {
        "ai": ["ai", "machine learning", "llm", "neural", "model", "intelligent"],
        "crypto": ["crypto", "blockchain", "web3", "defi", "solidity", "avm", "ethereum"],
        "infra": ["infrastructure", "devops", "kubernetes", "docker", "ci/cd", "aws", "azure"],
        "backend": ["backend", "api", "fastapi", "django", "server", "database", "postgresql", "mysql"],
        "frontend": ["frontend", "react", "next.js", "ui", "css", "tailwind", "vue"],
        "security": ["security", "auth", "encryption", "zero trust", "vulnerability", "penetration testing"]
    }
...
def get_readable_profile(self) -> str:
    ...
    Format: A short paragraph of natural language covering domain expertise,
    operational conduct, activity patterns, communication style, capabilities
    and active project context. toolpath is excluded (operational, agent-only
...
def run_profile_render(self) -> int:
    ...
    Called by DistillationProcess as the final step of the maintenance pass,
    after run_landmark_decay().
...
classDiagram
    ...
    MyceliumDatabase -- Landmark : _auto_connect_landmark
    MyceliumDatabase -- Landmark : save
    MyceliumDatabase -- Landmark : find_matching
...
class ProfileRenderer:
    ...
    The profile is the translation of the map into human language.
    The map is the truth. The profile is the projection.

    It is never manually written or edited. It is always re-derived.
    When coordinates shift, the profile regenerates to match.
    ...
    2. Agent context - an optional `user_profile` zone in MemoryInterface,
    injected only when the task requires high personalization and the coordinate path alone is insufficient. NOT injected on every call.
    """
    ...
    for section_id, space_id in dirty_rows:
        ...
        if section_id:
            ...
            self.conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?)
            """, (section_id, space_id, 0, prose, json.dumps(node_ids),
                  json.dumps(lm_ids), False, time.time(), len(prose.split())))
...
Sits between EpisodicStore and MyceliumInterface. Neither side owns it. Two classes:

EpisodeIndexer (writes the bridge table at episode store time) and ResonanceScorer (augments cosine similarity with coordinate overlap on retrieval).
...
def index_episode(self, episode_id: str, session_id: str,
    ...
    episode_id: the episode just written to episodes table
    session_id: the session that produced the episode
    landmark_id: the landmark crystallized from this session (may be None
...
def augment_retrieval(
    ...
    for ep in candidates:
        ...
        ep["resonating_spaces"] = [space_id for space_id, _ in current_spaces if space_id in self._get_resonating_spaces(index)]
```
```python
def _calculate_resonance_score(index, current_spaces):
    """
    Calculate the resonance score for an episode based on its index and the current spaces.
    """
    resonance_score = 0.0
    if index is not None:
        for space_id, _ in current_spaces:
            if space_id in self._get_resonating_spaces(index):
                resonance_score += self._get_resonance_weight_per_s()
    return resonance_score
```
```python
def _is_suppressed(index, current_spaces):
    """
    Check if an episode is suppressed based on its index and the current spaces.
    """
    if index is None:
        return False
    for space_id, _ in current_spaces:
        if space_id in self._get_suppressed_spaces(index):
            return True
    return False
```
```python
def _get_resonating_spaces(index):
    """
    Get the list of space_ids that resonated with the given index.
    """
    if index is None:
        return []
    return self._get_resonating_spaces(index)
```
```python
def _get_suppressed_spaces(index):
    """
    Get the list of space_ids that are suppressed for the given index.
    """
    if index is None:
        return []
    return self._get_suppressed_spaces(index)
...
"""
What fraction of the episode's coordinate nodes are already present in the current session's active nodes?
