...
class WakeWordDetector:
    ...
    def __init__(
        ...
        gain: float = 3.0  # Moderate gain to boost low microphone input (was 50.0 which was too high)
    ):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not model_key:
                logger.info(f"[WakeWordDetector] Phrase '{self.wake_phrase}' not in PRETRAINED_MODELS. Trying Jarvis...")
            ...
            try:
                ...
                logger.info(f"[WakeWordDetector] Successfully initialized with: {self.wake_phrase}")
                return True
            except Exception as e:
                logger.info(f"[WakeWordDetector] Failed to load {model_key}: {e}. Trying fallback to Jarvis...")
                self._oww = Model(
            ...
            logger.warning("[WakeWordDetector] openwakeword not installed")
            logger.info("[WakeWordDetector] Run: pip install openwakeword")
            return False
        except Exception as e:
            logger.error(f"[WakeWordDetector] Initialization failed: {e}")
            import traceback
            traceback.print_exc()
            return False
            ...
            if prediction:
                for model_name, score in prediction.items():
                    if score >= self.sensitivity:
                        logger.info(f"[WakeWordDetector] Detected {model_name} with score {score:.4f}")
                        return True
            
            return False
            
        except Exception as e:
            logger.error(f"[WakeWordDetector] Processing error: {e}")
            import traceback
            traceback.print_exc()
            return False
