...
class TTSManager:
    ...
    def update_config(self, **kwargs) -> None:
        ...
        logger.info(f"[TTSManager] Updated config: {kwargs}")
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.warning("[TTSManager] OpenAI package not installed")
                return None
            except Exception as e:
                logger.error(f"[TTSManager] Failed to create OpenAI client: {e}")
                return None
        return self._client
    ...
    def _get_local_engine(self):
        ...
        if self._local_engine is None:
            try:
                ...
                logger.error(f"[TTSManager] Failed to initialize pyttsx3: {e}")
                return None
    ...
    def synthesize(self, text: str) -> Optional[np.ndarray]:
        ...
        if voice_config == "Built-in":
            try:
                logger.info(f"[TTSManager] Using local Built-in voice for: {text[:50]}...")
                ...
                logger.error(f"[TTSManager] Local synthesis error: {e}")
                return None

        # OPENAI TTS
        client = self._get_client()
        if not client:
            logger.warning("[TTSManager] OpenAI client not available")
            return None
        
        try:
            logger.info(f"[TTSManager] Synthesizing: {text[:50]}...")
            ...
            if audio_segment.frame_rate != 16000:
                logger.info(f"[TTSManager] Resampling from {audio_segment.frame_rate}Hz to 16000Hz")
            ...
            logger.info(f"[TTSManager] Generated {len(audio)} samples at {audio_segment.frame_rate}Hz")
            return audio
            
        except ImportError:
            logger.warning("[TTSManager] pydub not installed, cannot decode MP3")
            return None
        except Exception as e:
            logger.error(f"[TTSManager] Synthesis error: {e}")
            return None
    ...
    def synthesize_to_file(self, text: str, output_path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"[TTSManager] Saved audio to {output_path}")
            return True
            
        except Exception as e:
            logger.error(f"[TTSManager] Synthesis to file error: {e}")
            return False
    ...
    def apply_pitch_shift(self, audio: np.ndarray, semitones: int) -> np.ndarray:
        ...
        try:
            ...
            logger.info("[TTSManager] librosa not installed, pitch shift skipped")
            return audio
        except Exception as e:
            logger.error(f"[TTSManager] Pitch shift error: {e}")
            return audio
