...
class MCPClient:
    ...
    async def connect_stdio(self, command: str, args: List[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"[MCPClient] Started {self.server_name} via stdio")
            ...
            logger.error(f"[MCPClient] Failed to start {self.server_name}: {e}")
            return False
    ...
    async def connect_websocket(self, url: str) -> bool:
        ...
        try:
            ...
            logger.info(f"[MCPClient] Connected to {self.server_name} via WebSocket")
            ...
            logger.error(f"[MCPClient] Failed to connect to {url}: {e}")
            return False
    ...
    async def connect_http(self, url: str) -> bool:
        ...
        try:
            ...
            logger.info(f"[MCPClient] Connected to {self.server_name} via HTTP")
    ...
    async def _initialize(self) -> bool:
        ...
        if response and response.result:
            ...
            logger.info(f"[MCPClient] Initialized {self.server_name}")
    ...
    async def _send_request(self, request: MCPRequest) -> Optional[MCPResponse]:
        ...
        try:
            ...
            logger.error(f"[MCPClient] Request {request.id} timed out")
    ...
    async def _read_stdio_messages(self):
        ...
        while self._process and self._process.stdout:
            try:
                ...
                logger.error(f"[MCPClient] stdio read error: {e}")
    ...
    async def _read_ws_messages(self):
        ...
        while self._ws:
            try:
                ...
                logger.info(f"[MCPClient] WebSocket closed")
                break
            except Exception as e:
                logger.error(f"[MCPClient] WebSocket read error: {e}")
    ...
    async def list_tools(self) -> List[MCPTool]:
        ...
        if response and response.result and "tools" in response.result:
            ...
            logger.info(f"[MCPClient] {self.server_name} has {len(self._tools)} tools")
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if response:
            if response.error:
                logger.error(f"[MCPClient] Tool error: {response.error}")
                return None
    ...
    async def disconnect(self):
        ...
        self._initialized = False
        logger.info(f"[MCPClient] Disconnected from {self.server_name}")
