...
class StateManager:
    ...
    async def update_app_state(self, app_state: IRISState) -> None:
    ...
    async def go_back(self, session_id: str) -> None:
        ...
        state_manager = await self._get_session_state_manager(session_id)
        if state_manager:
            await state_manager.go_back()

    async def collapse_to_idle(self, session_id: str) -> None:
        """Collapse the UI to idle for a session"""
        state_manager = await self._get_session_state_manager(session_id)
        if state_manager:
            await state_manager.collapse_to_idle()
