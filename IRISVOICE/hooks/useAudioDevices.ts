...
export function useAudioDevices(): UseAudioDevicesReturn {
  ...
  const fetchDevices = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        // 404 means endpoint doesn't exist yet - use fallback silently
        if (response.status === 404) {
          console.log("[useAudioDevices] Backend endpoint not available, using fallback devices")
          setInputDevices(["Default", "USB Microphone", "Headset", "Webcam"])
          setOutputDevices(["Default", "Headphones", "Speakers", "HDMI"])
          setIsLoading(false)
          return
        }
