...
### Phase 5 — Active Caducean Governance
* Modulate planning temperature in `_plan_task` based on Caducean state `u`.
* Integrate Caducean recommendation integers (`0`=EXPAND, `1`=COMPRESS) in the loop termination logic.
* Set up parameter tuning via `tune_dffing_params` on topological violations.
