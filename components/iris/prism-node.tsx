...
interface PrismNodeProps {
  node: { id: string; label: string; icon: ElementType; fields?: Record<string, unknown>[] }
  angle: number
  radius: number
  nodeSize: number
...
export function PrismNode({
  ...
  const baseTransition = {
    ...
    ease: spinConfig.ease as [number, number, number, number],
  }
  ...
  return (
    <motion.button
      className={`absolute flex flex-col items-center justify-center cursor-pointer z-10 pointer-events-auto ${isChatActive ? 'opacity-50 pointer-events-none' : ''}`}
      style={{
        left: "50%",
        top: "50%",
      ...
      whileHover={{ scale: isChatActive ? 1.0 : 1.05 }}
