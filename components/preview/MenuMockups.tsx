...
// ── Mockup 11: Dock Redesign (arcs span full dock height, tight inward curve) ───────
function MockupDockRedesign({ glowColor }: { glowColor: string }) {
  const [hoveredId, setHoveredId] = useState<string | null>(null)
  const btnW = 40
  const dockH = 44 // same as original Dock (py-2 = 8+28+8)
  const arcStroke = 2.5
  // Arc spans from near top of dock to near bottom, curving inward
  const arcTop = 3        // 3px from dock top
  const arcBottom = dockH - 3 // 3px from dock bottom
  const arcCtrlX = btnW * 0.35 // control point pulled inward (tighter curve)
  const arcCtrlY = dockH * 0.38 // control point sits ~38% from top
  const arcPath = `M 2 ${arcTop} Q ${arcCtrlX} ${arcCtrlY} 2 ${arcBottom}`
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <div key={cat.id}
                ...
                {/* Arc — absolute, spans full dock height, curves inward */}
                <div style={{
                  position: 'absolute',
                  top: 0,
                  left: 0,
                  ...
                  <svg width={btnW} height={dockH} viewBox={`0 0 ${btnW} ${dockH}`}>
                    ...
                    <path d={arcPath} fill="none"
                      stroke={`url(#dock-arc-${cat.id})`} strokeWidth={arcStroke}
                ...
                {/* Icon nestled at center of arc curve */}
                <div style={{
                  position: 'relative',
                  width: btnW,
                  height: dockH,
